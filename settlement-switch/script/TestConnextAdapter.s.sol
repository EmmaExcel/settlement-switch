...
contract TestConnextAdapter is Script {
    ...
    function run() external {
        ...
        console.log("Testing Connext Adapter...");
        console.log("Connext Adapter address:", CONNEXT_ADAPTER);

        IBridgeAdapter connextAdapter = IBridgeAdapter(CONNEXT_ADAPTER);

        // Test 1: Check if adapter supports the route
        console.log("\n=== Testing Route Support ===");
        try connextAdapter.supportsRoute(
            WETH_ADDRESS,
            WETH_ADDRESS,
            SRC_CHAIN_ID,
            DST_CHAIN_ID
            ...
            if (supported) {
                console.log("[SUCCESS] Connext adapter supports WETH Sepolia -> Arbitrum Sepolia route");
            } else {
                console.log("[INFO] Connext adapter does NOT support WETH Sepolia -> Arbitrum Sepolia route");
                ...
                try connextAdapter.supportsRoute(
                    ...
                    if (ethSupported) {
                        console.log("[SUCCESS] Connext adapter supports ETH Sepolia -> Arbitrum Sepolia route");
                    } else {
                        console.log("[INFO] Connext adapter does NOT support ETH route either");
                    }
                } catch {
        ...
        console.log("\n=== Testing Liquidity ===");
        try connextAdapter.getAvailableLiquidity(
            WETH_ADDRESS,
            WETH_ADDRESS,
            SRC_CHAIN_ID,
            DST_CHAIN_ID
        ...
        try connextAdapter.getRouteMetrics(
            WETH_ADDRESS,
            WETH_ADDRESS,
            BRIDGE_AMOUNT,
            SRC_CHAIN_ID,
            ...
            console.log("[SUCCESS] Connext route metrics retrieved:");
        ...
        try connextAdapter.isHealthy() returns (bool healthy) {
            if (healthy) {
                console.log("[SUCCESS] Connext adapter is healthy");
            } else {
                console.log("[WARNING] Connext adapter is not healthy");
            }
        } catch Error(string memory reason) {
        ...
        try connextAdapter.getTransferLimits(
            WETH_ADDRESS,
            SRC_CHAIN_ID,
            DST_CHAIN_ID
        ) returns (uint256 minAmount, uint256 maxAmount) {
            console.log("[SUCCESS] Connext transfer limits:");
        ...
        try connextAdapter.getSuccessRate(
            SRC_CHAIN_ID,
            DST_CHAIN_ID
        ) returns (uint256 successRate) {
            console.log("[SUCCESS] Connext success rate:", successRate, "%");
        } catch Error(string memory reason) {
        ...
        console.log("\nConnext Adapter testing completed!");
