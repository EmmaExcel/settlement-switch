...
export default function Home(){
  ...
  return (
    <>
      <section className="w-full flex-1 flex justify-center py-6 items-center h-[800px] bg-[#f7f6f8] ">
     <div className="flex flex-col items-center gap-y-4">
      ...
      <Link href={"/bridge"} className="bg-purple-600 text-white px-6 py-3 rounded-full text-[18px] font-medium cursor-pointer hover:scale-95 scale-100 hover:duration-200 duration-150">Start bridging now</Link>
     </div>
    </section>

     <section className="py-2 flex justify-center bg-[#f7f6f8]">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-0 max-w-5xl ">
        {features.map((feature, index) => (
          <div key={index} className="flex flex-1 gap-3 rounded-lg border border-border bg-card p-6 h-72 flex-col items-center text-center justify-center">
            <feature.icon className="text-purple-800 w-8 h-8" />
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-bold leading-tight text-foreground">{feature.title}</h2>
              <p
                className="text-base font-normal leading-normal text-text-subtle"
     ...
     <section className="py-2 flex flex-col justify-center items-center pt-16 bg-[#f7f6f8]">
      <div className="pb-10">
        <h2 className="text-2xl font-bold leading-tight tracking-[-0.015em] px-4  pt-5 text-center text-foreground">
        How It Works
      </h2>
      <p className="text-text-subtle-light dark:text-text-subtle-dark max-w-xl mx-auto">Get started in three simple steps. Bridging has never been easier.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-0 max-w-5xl ">
        {steps.map((step, index) => (
          <div
            key={index}
            className="flex flex-1 gap-3 rounded-lg border border-border bg-card p-6 h-72 flex-col items-center text-center justify-center"
          >
           <step.icon className="text-purple-800 w-8 h-8" />
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-bold leading-tight text-foreground">{step.title}</h2>
              <p
                className="text-base font-normal leading-normal text-text-subtle"
                style={{ color: "var(--text-subtle)" }}
              >
